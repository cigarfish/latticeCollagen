...
template <int Dimensions, class VertexType>
inline void Simplex<Dimensions, VertexType>::getCircumCenter3D( POSITION_T center[3])
{
...
template <int Dimensions, class VertexType>
inline void Simplex<Dimensions, VertexType>::getCircumCenter2D( POSITION_T center[2])
{
	//Simplex<Dimensions, VertexType> *tet = this;
...
void Triangulation<Dimensions,VertexType,SimplexType>::setDomain()
{
	if( !domainSet){
		...
		boundaryThickness = pow( (double) (boundaryThickness / countVertices) ,1./Dimensions);
...
void Simplex<Dimensions,VertexType>::getCircumsphereRadius( long double *circumsphere, long double &radius)
{
	...
	int k, l;
	const int NR_Simplex_POINTS = Dimensions+1;
		...
		bool printPoints, bool printFramePoints, bool printNeighborship, bool printTetrahedra, bool printConvexHull)
{
	...
	fprintf( fp, "#include \"finish.inc\"\n"\
				...
				0.5*(domain->_min(0)+domain->_max(0)), 0.5*(domain->_min(1)+domain->_max(1)), MAX(domain->_max(0)-domain->_min(0), domain->_max(1)-domain->_min(1)),
				0.5*(domain->_min(0)+domain->_max(0)), 0.5*(domain->_min(1)+domain->_max(1)));
...
template <int Dimensions, class VertexType, class SimplexType>
int Triangulation<Dimensions,VertexType,SimplexType>::getCountConvexFaces()
{return this->countConvexFaces;}

template <int Dimensions, class VertexType, class SimplexType>
int Triangulation<Dimensions,VertexType,SimplexType>::getCountVertices()
{return this->countVertices;}
...
inline void Triangulation<Dimensions,VertexType,SimplexType>::setFramePoints3D()
{
	...
	double strech = MAX( triangulation->domain->_max(0) - triangulation->domain->_min(0),
			MAX( triangulation->domain->_max(1) - triangulation->domain->_min(1),
					triangulation->domain->_max(2) - triangulation->domain->_min(2)));
	...
	for( i=0; i<(int)pow( 2., 3); i++){
		...
		for( dim=0; dim<3; dim++){
			...
			triangulation->framePoints[triangulation->countFramePoints]->pos()[dim] = (1 - temp%2)*triangulation->domain->_min(dim) + (temp%2)*triangulation->domain->_max(dim) + (2*(temp%2)-1)*myRand()*strech;
			temp = temp/2.;
		}
...
inline void Triangulation<Dimensions,VertexType,SimplexType>::setFramePoints2D()
{
	...
	for( int i=0; i<(int)pow( 2., 2); i++){
		...
		for( int dim=0; dim<2; dim++){
			...
			//printf( "%lf ", (1 - temp%2)*xMin[dim] + (temp%2)*xMax[dim] + ((temp%2 - 0.5)));
			//printf( "%lf ", (1 - temp%2)*xMin[dim] + (temp%2)*xMax[dim] + (2*(temp%2)-1)*myRand()*1.);
			this->framePoints[this->countFramePoints]->pos()[dim] = (1 - temp%2)*domain->_min(dim) + (temp%2)*domain->_max(dim) + ( i%2 != i/2 ? (2.*(temp%2)-1.)*myRand() : (2.*(temp%2)-1.)*(myRand()+1));
			temp = temp/2;
		}
		//fprintf( stderr, " (%i)", countOnes);
...
inline void Triangulation<Dimensions,VertexType,SimplexType>::setFramePoints1D()
{
	...
	for( int i=0; i<1+1; i++){
		...
		triangulation->framePoints[i]->pos()[0] = (i? triangulation->domain->_min(0)-myRand() : triangulation->domain->_max(0)+myRand());
	}
...
void Triangulation<Dimensions, VertexType, SimplexType>::printVoronoiDiagramToPovray3D( const char * filename, Domain<Dimensions>* subDomain)
{
	...
	PovrayIO::writePovrayHeader( &fs, domain->_min(0), domain->_min(1), domain->_max(0), domain->_max(1), 2*(domain->_max(0)-domain->_min(0)));
...
inline void Triangulation<Dimensions, VertexType, SimplexType>::printVoronoiDiagramToEps2D( const char * filename)
{
	...
	std::fstream fs;
	fs.open(filename, std::fstream::out);
	EPS::PSwriteHeader(&fs, domain->_min(0) * scale,
			domain->_max(0) * scale, domain->_min(1) * scale,
			domain->_max(1) * scale);
...
template <int Dimensions>
POSITION_T Domain<Dimensions>::_min( int dimension)
{	return xMin[dimension];}

template <int Dimensions>
POSITION_T Domain<Dimensions>::_max( int dimension)
