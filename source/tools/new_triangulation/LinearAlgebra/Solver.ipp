...
// If A is symmetric positive matrix
{
	...
	//T *r_hat[l+1];
	//T *u_hat[l+1];
	T **r_hat = (T**) malloc( sizeof(T*)*(l+1));
	T **u_hat = (T**) malloc( sizeof(T*)*(l+1));
	...
	T *Sigma = (T*) malloc( sizeof(T)*(l+1));
	T *Gamma = (T*) malloc( sizeof(T)*(l+1));
	T *Gamma_prime = (T*) malloc( sizeof(T)*(l+1));
	T *Gamma_prime2 = (T*) malloc( sizeof(T)*(l+1));

	T **Tau = (T**) malloc( sizeof(T*)*(l));
	for( int i = 0 ; i < (l+1) ; i++){
		Tau[i] = (T*) malloc( sizeof(T)*(l+1));
		//T Tau[l][l+1];
	}
	// INIT
