...
#include <stdio.h>
#include <math.h>
//commented by Adrian: not Windows compatible!
	...
	for( int xi=0; xi<vesselGraph->DOMAIN_SIZE_X;xi += t->LATTICE_CONSTANT/vesselGraph->LATTICE_CONSTANT)
		for( int yi=0; yi<vesselGraph->DOMAIN_SIZE_Y; yi += t->LATTICE_CONSTANT/vesselGraph->LATTICE_CONSTANT)
			for( int zi=0; zi<vesselGraph->DOMAIN_SIZE_Z; zi += t->LATTICE_CONSTANT/vesselGraph->LATTICE_CONSTANT){
				if( vesselGraph->octree->firstBetween( 	xi, yi, zi,
														xi+t->LATTICE_CONSTANT/vesselGraph->LATTICE_CONSTANT,
														yi+t->LATTICE_CONSTANT/vesselGraph->LATTICE_CONSTANT,
														zi+t->LATTICE_CONSTANT/vesselGraph->LATTICE_CONSTANT) == 0){
...
VoronoiDiagram *createVoronoiTesselationHexagonal( VesselGraph *vesselGraph, float latticeConstant = 1.){
	VoronoiDiagram *t = new VoronoiDiagram();
	...
	*/
	...
	float center[3];
	center[0] = vesselGraph->DOMAIN_SIZE_X/2.*vesselGraph->LATTICE_CONSTANT/t->LATTICE_CONSTANT;
	center[1] = vesselGraph->DOMAIN_SIZE_Y/2.*vesselGraph->LATTICE_CONSTANT/t->LATTICE_CONSTANT;
	center[2] = vesselGraph->DOMAIN_SIZE_Z/2.*vesselGraph->LATTICE_CONSTANT/t->LATTICE_CONSTANT;
...
//					if( (yi!=0 && zi!=0 ) )
					...
					if( (150./vesselGraph->LATTICE_CONSTANT)
						   > std::sqrt(  (yi+t->LATTICE_CONSTANT/vesselGraph->LATTICE_CONSTANT-center[1])
										*(yi+t->LATTICE_CONSTANT/vesselGraph->LATTICE_CONSTANT-center[1])
										+(zi+t->LATTICE_CONSTANT/vesselGraph->LATTICE_CONSTANT-center[2])
										*(zi+t->LATTICE_CONSTANT/vesselGraph->LATTICE_CONSTANT-center[2])) )
...
//commented by Adrian: not windows compatible: see sys/* includes
#ifndef __BUILD_WINDOWS__
...
int perfusion(int argc, char** argv, VesselGraph *vesselGraph) {
	...
	char c;
	while ((c = getopt(argc, argv, "ABPf::p:x:y:z:r:n:T:NV:OI:i:S:1:2:R:t:e:o:d:C:D:X:c:W:E::sa:b:F")) != -1)
		switch (c) {
		case 'F':{
		...
		case 'D':{
			...
			FILE *fp;
			char filename[512];
			sprintf( filename, "parameters.dat", dirname);
		...
		case 'I':{
			...
			if(strstr(optarg,"singleStraight")){
				...
				vesselGraph->setSingleVessel(
						vesselGraph->DOMAIN_SIZE_X-1, vesselGraph->DOMAIN_SIZE_Y/2, vesselGraph->DOMAIN_SIZE_Z/2);
			}else
			if(strstr(optarg,"singleRandom")){
				fprintf(stderr, "Set Single (Random) Vessel\n");
				vesselGraph->setSingleRandomVessel(
						vesselGraph->DOMAIN_SIZE_X-1, vesselGraph->DOMAIN_SIZE_Y/2, vesselGraph->DOMAIN_SIZE_Z/2);
			}else{
		...
		case 'O':
		{
			...
			//if( vesselGraph->dimensions < 3)
			{
			vesselGraph->printFlowMap((char*) "flow.60x60x60.eps", vesselGraph, 60., 60.,
					60.);
			vesselGraph->printBloodVolumeFractionMap((char*) "f_p.60x60x60.eps", vesselGraph,
					60., 60., 60.);
			vesselGraph->printInterstitialSpaceVolumeFractionMap((char*) "f_i.60x60x60.eps",
					vesselGraph, 60., 60., 60.);
			vesselGraph->printPermeabilityMap((char*) "Kps.60x60x60.eps", vesselGraph, 60.,
					60., 60.);
			}
